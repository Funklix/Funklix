...
assert.deepStrictEqual(CALLBACK_PHASES,require('../api/social-connector/oauth-phases').OAUTH_PHASES);assert(CALLBACK_PHASES.includes('state_received'));assert(phases.every(phase=>CALLBACK_PHASES.includes(phase)));
...
(async()=>{
 ...
 const schema=fs.readFileSync('api/social-connector/schema.js','utf8');assert(schema.includes("UPDATE public.social_oauth_attempts SET return_path='/' WHERE return_path IS DISTINCT FROM '/'"));assert(schema.includes("conname='social_oauth_attempts_last_confirmed_phase_check'"));assert(!schema.includes('DROP CONSTRAINT IF EXISTS social_oauth_attempts_last_confirmed_phase_check'));
