...
(async()=>{
 ...
 const schema=fs.readFileSync('api/social-connector/schema.js','utf8');assert(schema.includes("UPDATE social_oauth_attempts SET return_path='/' WHERE return_path='/settings'"));assert(schema.includes("conname='social_oauth_attempts_last_confirmed_phase_check'"));assert(!schema.includes('DROP CONSTRAINT IF EXISTS social_oauth_attempts_last_confirmed_phase_check'));
