...
for(const step of ['schema_lock_acquire','transaction_begin','oauth_return_path_normalization','oauth_return_path_constraint_inspect','oauth_connection_association','schema_version_write','schema_commit','transaction_rollback'])assert(schema.STEPS.includes(step));
...
const diagnostic=settings.unavailable('req_safe',{phase:'schema_initialization',operation_category:'oauth_return_path_normalization',database_error_category:'check_violation'}).diagnostic;
assert.deepStrictEqual(Object.keys(diagnostic).sort(),[...settings.DIAGNOSTIC_FIELDS].sort());assert.strictEqual(diagnostic.operation_category,'oauth_return_path_normalization');assert.strictEqual(diagnostic.database_error_category,'check_violation');assert(!JSON.stringify(diagnostic).includes('UPDATE '));
async function integration(){
 ...
 try{
  ...
  const first=await schema.ensureSocialConnectorSchema(pool,{});assert(first.changed);const second=await schema.ensureSocialConnectorSchema(pool,{});assert.strictEqual(second.version,4);
  const version=await pool.query('SELECT version FROM social_connector_schema_version');assert.strictEqual(version.rows[0].version,4);
