...
const tabs=[...host.innerHTML.matchAll(/role="tab"[^>]*data-cw-mode="([^"]+)"/g)].map(x=>x[1]);assert.deepStrictEqual(tabs,['library','calendar'],'two coherent content-planning destinations');
...
w.calendarState.mode='calendar';w.calendarState.view='list';w.render(host,context);assert.match(host.innerHTML,/content-calendar-list/);assert.match(host.innerHTML,/Unscheduled backlog/);assert.match(host.innerHTML,/Eligible/);assert.match(host.innerHTML,/Planning only\. Posts are not published automatically/);
...
for(const source of ['canvas','inspector','content_library','review_queue','calendar'])assert.ok(w.PLANNING_SOURCES.includes(source));assert.equal(w.evaluateScheduling({node:social('model-c','Draft'),accountId:'a',boardId:'b',canEdit:true,warningAccepted:true}).canPlan,true);
