...
assert.deepStrictEqual(CALLBACK_PHASES,require('../api/social-connector/oauth-phases').OAUTH_PHASES);assert(CALLBACK_PHASES.includes('state_received'));
...
const app=fs.readFileSync('app.js','utf8'),html=fs.readFileSync('index.html','utf8'),lang=fs.readFileSync('language.js','utf8'),schema=fs.readFileSync('api/social-connector/schema.js','utf8'),adapter=fs.readFileSync('api/social-connector/linkedin-adapter.js','utf8'),settings=fs.readFileSync('social-connections-settings.js','utf8');
for(const text of ["body:JSON.stringify({returnPath:'/'})",'showModal()','history.replaceState','callbackFeedback','Copy diagnostics'])assert(app.includes(text)||html.includes(text)||settings.includes(text));
