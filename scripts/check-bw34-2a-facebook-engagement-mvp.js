...
(async()=>{
 ...
 let calls=0,url,options;const adapter=createFacebookAdapter({fetchImpl:async(u,o)=>{calls++;url=String(u);options=o;return{ok:true,text:async()=>JSON.stringify({reactions:{summary:{total_count:0}},comments:{summary:{total_count:0}}})};}});const response=await adapter.facebook_post_engagement_read_v1({context:{requestId:'r'},credentials:{pageTokens:{'123':'invented-token'}},input:{pageId:'123',postId:'123_456'}});assert(response.ok);assert.equal(calls,1);assert(url.includes('/v24.0/123_456?'));assert.equal(new URL(url).searchParams.get('fields'),'reactions.limit(0).summary(true),comments.limit(0).summary(true),shares');assert.equal(options.method,'GET');assert(!url.includes('invented-token'));assert.equal(options.headers.authorization,'Bearer invented-token');
 ...
 assert(ui.includes('facebookEngagementRequests.has(nodeId)'));assert(ui.includes('finalizedFacebookByNode.has(nodeId)'));assert(ui.includes('Published to Facebook')&&ui.includes('Open on Facebook'));assert(!ui.includes('setInterval('));
