...
assert(renderer.indexOf('"Canvas Diagnostics"') < renderer.indexOf('"Measured performance"'), "BW-28.3 diagnostic and collapsed measured sections are not ordered separately");
...
for (const text of ["Current Canvas","Deterministic diagnostic","Includes unsaved Canvas changes","Based on the currently loaded saved Canvas","Canvas readiness","Campaign journey coverage","CTA variation among existing CTAs","ICP consistency","Tone consistency","Trust-layer coverage"]) assert(app.includes(text), `missing diagnostic contract: ${text}`);
for (const guard of ["state.boardLoadState","!state.currentBoardId","state.boardAccess?.canView === false","!state.nodes.length","isCurrentInsightsSnapshot"]) assert(renderer.includes(guard), `missing guarded state: ${guard}`);
