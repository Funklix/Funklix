...
for (const token of ['function applyContentWorkspaceTransition', 'resolveCurrentContentNode(prepared.nodeId, prepared)', 'workspace.evaluateTransition({ currentStatus: node.status', 'node.approvedContentFingerprint = workspace.materialFingerprint(node)', 'delete node.approvedContentFingerprint', 'recordStatusChangedActivity(node', 'updateNodeCard(node)', 'fillInspector(node)', 'updateListView()', 'markUnsaved()', 'window.FunklixContentWorkspace.openTransition(node.id, "Draft", el.inputs.status)']) assert(app.includes(token), token);
