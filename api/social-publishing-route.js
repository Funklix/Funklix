...
function validInput(body, publish = false) { const keys = ['boardId', 'nodeId', 'destinationId', 'clientRequestId', 'expectedApprovedFingerprint', 'expectedBoardRevision', 'saveConfirmationCategory', 'nodeMaterialNormalizationCategory', 'requestLifecycleGeneration', ...(publish ? ['confirmed'] : [])]; if (Object.keys(body).some(key => !keys.includes(key) || ['__proto__', 'prototype', 'constructor'].includes(key))) return false; const fingerprintValid = !publish || REF.test(body.expectedApprovedFingerprint || ''); return UUID.test(body.boardId || '') && REF.test(body.nodeId || '') && UUID.test(body.destinationId || '') && REF.test(body.clientRequestId || '') && fingerprintValid && (body.expectedApprovedFingerprint == null || typeof body.expectedApprovedFingerprint === 'string') && (body.expectedBoardRevision == null || (typeof body.expectedBoardRevision === 'string' && body.expectedBoardRevision.length <= 128)) && (!publish || body.confirmed === true); }
