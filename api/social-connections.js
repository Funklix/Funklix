...
function facebookProjection(configuration,rows){const connection=rows.find(x=>x.platform==='facebook'&&x.status!=='disconnected')||rows.find(x=>x.platform==='facebook');if(!configuration.ready)return{state:'setup_required',configured:false,connect_allowed:false,reconnect_allowed:false,disconnect_allowed:false,configuration_issue:configuration.readiness,destinations:[]};if(!connection||connection.status==='disconnected')return{state:'not_connected',configured:true,connect_allowed:true,reconnect_allowed:false,disconnect_allowed:false,destinations:[]};const destinations=(connection.destinations||[]).filter(d=>d.active).map(d=>({id:d.id,label:d.display_name,provider_identity:d.external_destination_id,type:d.destination_type,capabilities:d.capabilities}));return{state:connection.status==='connected'?'connected':'needs_attention',configured:true,connect_allowed:false,reconnect_allowed:true,disconnect_allowed:true,connection_id:connection.id,display_name:connection.external_display_name,permission_state:connection.status==='connected'?'authorized':'permission_expired',destinations};}
