...
function createPublishingService({ pool = defaultPool, adapter = createLinkedInAdapter(), env = process.env, now = () => new Date() } = {}) {
  ...
  async function publish(input, actor) {
    ...
    let result;
    try { result = await adapter.linkedin_text_publish_v1({ context: { requestId: input.serverRequestId, accountId: input.ownerAccountId }, credentials: credential, input: { author: state.destination.external_destination_id, caption: state.evaluation.caption } }); }
