...
const DIAGNOSTIC_FIELDS=Object.freeze(['server_request_id','classification','phase','operation_category','database_error_category','schema_target_version','schema_committed_version','schema_structural_state','schema_maintenance_state','build_identity','database_identity_digest','committed_state_category','timestamp']);
function unavailable(requestId,context={}){const {STEPS,SCHEMA_VERSION}=require('./schema');const operations=new Set([...STEPS,'connection_projection','credential_projection','destination_projection','oauth_attempt_projection','response_construction','storage_unknown']);const errors=new Set(['required_table_missing','required_column_missing','storage_permission_denied','storage_connection_failed','storage_timeout','data_exception','integrity_constraint_violation','transaction_retry_required','projection_failed','syntax_error','insufficient_privilege','duplicate_object','undefined_column','undefined_table','invalid_foreign_key','check_violation','unique_violation','dependent_objects','transaction_aborted','lock_failure','connection_failure','unknown_database_error']);const diagnostic={server_request_id:requestId,classification:'storage_unavailable',phase:context.phase||'storage_unknown',operation_category:operations.has(context.operation_category)?context.operation_category:'storage_unknown',database_error_category:errors.has(context.database_error_category)?context.database_error_category:'projection_failed',schema_target_version:SCHEMA_VERSION,schema_committed_version:Number.isInteger(context.schema_committed_version)?context.schema_committed_version:null,schema_structural_state:context.schema_structural_state||'unknown',schema_maintenance_state:context.schema_maintenance_state||'unknown',build_identity:context.build_identity||'bw32.2.12',database_identity_digest:context.database_identity_digest||null,committed_state_category:'unchanged',timestamp:new Date().toISOString()};return validate({...base('service_unavailable',true,[false,false,false]),diagnostic});}
