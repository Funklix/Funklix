...
const DIAGNOSTIC_FIELDS=Object.freeze(['server_request_id','classification','phase','operation_category','database_error_category','schema_version','committed_state_category','timestamp']);
function unavailable(requestId,context={}){const operations=new Set(['advisory_lock_acquire','transaction_begin','token_secrets_table','connected_accounts_table','destinations_table','oauth_attempts_table','publish_jobs_table','provider_attempts_table','external_posts_table','oauth_return_path_column','oauth_return_path_backfill','oauth_return_path_constraint','oauth_connection_column','oauth_connection_constraint','schema_indexes','schema_version_read','schema_version_write','transaction_commit','transaction_rollback','schema_unknown','connection_projection','credential_projection','destination_projection','oauth_attempt_projection','response_construction','storage_unknown']);const errors=new Set(['insufficient_privilege','duplicate_object','undefined_column','undefined_table','invalid_foreign_key','check_violation','unique_violation','dependent_objects','transaction_aborted','lock_failure','connection_failure','unknown_database_error']);const diagnostic={server_request_id:requestId,classification:'storage_unavailable',phase:context.phase||'storage_unknown',operation_category:operations.has(context.operation_category)?context.operation_category:'storage_unknown',database_error_category:errors.has(context.database_error_category)?context.database_error_category:'unknown_database_error',schema_version:'3',committed_state_category:'unchanged',timestamp:new Date().toISOString()};return validate({...base('service_unavailable',true,[false,false,false]),diagnostic});}
