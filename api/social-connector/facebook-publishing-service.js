...
function createFacebookPublishingService({ pool = defaultPool, adapter = createFacebookAdapter(), env = process.env, now = () => new Date() } = {}) {
  ...
  async function publish(input, actor) {
    ...
    try {
      ...
      return { ok: true, status: 'published', jobId, providerAttemptId: attemptId, publishedAt, destination: { type: 'page', label: state.destination.display_name }, externalUrl: url };
