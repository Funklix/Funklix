...
module.exports=async function handler(req,res){
  ...
  if(!contract.validateRequest(body).ok)return send(res,400,diag);
  ...
  const canvas=validateCanvasContext({nodes:body.canvas_context.nodes,edges:body.canvas_context.edges},null);if(!canvas.ok)return send(res,canvas.status===413?413:canvas.status===409?409:400,{...diag,code:canvas.status===409?'canvas_context_changed':'invalid_request',classification:'canvas_context_invalid'});
  let resolvedCanvas=canvas;if(body.canvas_context.saved_state==='saved'&&board.canvas_json){const authoritative=validateCanvasContext({nodes:board.canvas_json.nodes||[],edges:board.canvas_json.edges||[]},null);if(!authoritative.ok)return send(res,409,{...diag,code:'canvas_context_changed',classification:'canvas_context_invalid'});resolvedCanvas=authoritative;}diag.phase='context_loaded';
  const revision=board.brand_core_source_revision||board.brand_core_source_updated_at||board.updated_at||'0',available=contract.projectTargetGroups(board.brand_core_snapshot,revision),evaluation=contract.evaluateConfiguration(body.configuration,available,resolvedCanvas.nodes);
  ...
  const selectedNodes=[...new Set(body.configuration.stages.flatMap(stage=>stage.node_ids))].map(id=>nodeMap.get(id)),context={groups,nodes:selectedNodes,stages:body.configuration.stages},controller=new AbortController(),timer=setTimeout(()=>controller.abort(),30000);diag.phase='provider_called';
  try{const output=await simulation.callProvider(context,body.response_language,controller.signal);diag.phase='provider_response_received';if(output?.response_language!==undefined&&output.response_language!==body.response_language)return send(res,502,{...diag,code:'response_language_mismatch',classification:'provider_response_invalid'});const validated=simulation.validateProviderOutput(output,groups,body.configuration.stages,nodeMap,output?.response_language===undefined?undefined:body.response_language);if(!validated)return send(res,502,{...diag,code:'response_invalid',classification:'provider_response_invalid'});diag.phase='response_validated';if(output?.response_language===undefined){diag.phase='completed';return send(res,200,{...diag,code:'success',classification:'synthetic_persona'},{simulation_id:body.client_run_id,response_language:body.response_language,classification:'synthetic_persona',personas:validated.personas,journeys:validated.journeys,aggregate_insights:validated.aggregate_insights,modeled_ranges:contract.aggregateRanges(validated.journeys,body.configuration.stages),changes_made:false});}
    ...
    return send(res,200,{...diag,code:'success',classification:'synthetic_persona'},{simulation_id:body.client_run_id,response_language:body.response_language,disclosure:body.response_language==='de'?'Simuliertes Ergebnis, keine gemessene Performance oder Vorhersage.':'Simulated result, not measured performance or a prediction.',selected_target_groups:groups,selected_stages:body.configuration.stages,personas,journeys,aggregate_insights,modeled_ranges:contract.aggregateRanges(journeys,body.configuration.stages),context:{board_id:board.id,board_name:board.name,board_revision:String(board.updated_at||''),canvas_revision:String(body.canvas_context.revision),saved_state:body.canvas_context.saved_state,stage_mapping_version:'bw28-v1',method_version:'bw29.4.1-v1'},changes_made:false});
