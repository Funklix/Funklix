...
module.exports = async function handler(req,res){
  ...
  try{
    ...
    return route.send(res,result.ok?200:409,{...result,confirmationToken,serverRequestId,clientRequestId:body.clientRequestId});
  }catch(error){
    const code=stage==='confirmation_binding'?'confirmation_binding_unavailable':stage==='request_body'?'request_body_unavailable':stage==='response_serialization'?'response_serialization_unavailable':'publishing_storage_unavailable';
