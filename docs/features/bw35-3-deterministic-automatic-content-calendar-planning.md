...
## BW-35.3R4 — batch runtime repair

### Proven runtime failure

The production Apply request resolved (rather than rejecting `fetch`) to the platform's not-found response for `PUT /api/boards/:id/posting-schedule/batch`. The repository had both `api/boards/[id]/posting-schedule.js` and a child function at `api/boards/[id]/posting-schedule/batch.js`; that file/directory route ownership is not a deployment-safe Vercel convention. Consequently the batch module did not boot, authentication and contract validation were never entered, no transaction began, and no database mutation occurred. The HTTP category was **404 route unavailable**; the client discarded the HTML/non-authoritative response as malformed and the studio collapsed it into the generic connection message.

The R1–R3 fixtures missed this because they inspected source strings or exercised local planner/session models. They neither resolved the production URL against a uniquely owned deployed function nor invoked the route, contract, service, transaction adapter, response parser, and reconciliation validator as one runtime chain. Ruled out by the R4 chain are method/content-type/credentials mismatch, malformed producer keys, Board-revision serialization, fingerprint generation, handler export shape, local import depth, transaction API/result shape, and success-envelope disagreement.

### Narrow correction and boundary

The sole canonical endpoint is now `PUT /api/boards/:id/posting-schedule-batch`, owned by `api/boards/[id]/posting-schedule-batch.js`; the conflicting nested function was removed. The shared browser runtime constructs the exact URL, same-origin credentials, JSON content type, request ID, and strict `posting_schedule_batch_v1` payload, then validates the complete authoritative identity/revision/count/schedule response before reconciliation. The route lazily loads production storage dependencies so its tracked-files-only module boot does not depend on an installed database package, while runtime requests retain the existing signed-session authentication. Board access queries now use the transaction client supplied by the batch service, preserving one transactional connection through access validation, the `FOR UPDATE` Board lock, validation, update, and commit.

Apply and complete-plan Undo each issue exactly one batch PUT. There is no retry, per-post request, provider request, Board refetch, or partial client reconciliation. Failures retain the proposal and configuration when safe and release pending state. The English/German taxonomy distinguishes rejected fetch/offline, route unavailable, authentication, permission, stale revisions, changed material, finalized publication, collision, invalid plan, server/storage failure, and malformed authoritative reconciliation.

Privacy-safe diagnostics contain only an allowlisted stage, bounded category, command count, lifecycle generation, and sanitized correlation ID. They cover Apply receipt/construction/dispatch, fetch resolution or rejection, HTTP category, route/authentication/contract/service/transaction stages, response parsing, reconciliation, and bounded failure. Bodies, captions, media URLs, fingerprints, credentials, provider identities, SQL, stacks, and full responses are excluded.

### Production-shaped regression, deployment, and rollback

`npm run check:bw35.3r4` generates a deterministic multi-post proposal, builds the production request, invokes the exported deployed handler with a signed invented session and production-shaped request/response objects, passes the real contract and service over a transaction adapter, validates the real response through the browser validator, and verifies one request, one transaction, one atomic update, advanced Board/schedule revisions, and zero provider operations. It also covers rejected fetch, 404, 401, 403, malformed input, stale Board and schedule revisions, material conflict, collision, finalized targets, storage rollback, malformed success, duplicate Apply, and lifecycle switch. The check copies only `git ls-files` into a directory without `node_modules`, boots and executes the route there, completes the success fixture, and runs browser-script integrity.

Deploy the route move, shared browser runtime, App/workspace wiring, transaction-client correction, regression, Runtime Boot Safety registration, and this documentation together. No database migration, dependency, CSS, global layout, provider configuration, or proposal-engine change is involved. Rollback these files as one unit; do not restore both old and new route owners simultaneously. Existing canonical schedules remain compatible.

### Manual browser acceptance

1. Deploy to a preview that uses the production Vercel routing convention and sign in with an editor account on an invented Board.
2. Open Content Calendar → Auto-plan, generate a multi-post proposal, and inspect the network panel without recording payload contents.
3. Click Apply twice rapidly. Confirm exactly one `PUT /api/boards/<id>/posting-schedule-batch`, JSON content type, same-origin session handling, a 200 bounded JSON response, one success toast, closed studio, and immediate Month/Week/List/count updates.
4. Click complete-plan Undo once and confirm exactly one additional batch PUT and atomic restoration.
5. Exercise offline, missing preview route, signed-out, viewer, stale revision, changed content, collision, finalized post, and forced storage failure cases. Confirm the specific localized message, retained proposal/configuration where safe, released pending state, retryability where safe, and no client authority change.
6. Repeat in English and German and switch Board during a pending response. Confirm late authority is ignored and no body lock, portal, focus trap, or unrelated dashboard/navigation/calendar layout state is altered.
7. Confirm network inspection contains zero Facebook, LinkedIn, Meta, Instagram, TikTok, X, CDN, or other provider mutation request.
