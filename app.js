...
function getDashboardKnowledgeInputStatus() {
  ...
  return DASHBOARD_KNOWLEDGE_INPUTS.map((label) => {
    const definition = getCanonicalMissingKnowledgeModuleDefinition(label);
    const hasTypedTile = definition ? customTiles.some((tile) => (
      isValidBrandCustomTile(tile)
      && !isMalformedGeneratedCustomTile(tile)
      && getCustomTileModuleType(tile) === definition.id
    )) : false;
    return {
      label,
      exists: hasTypedTile || searchable.includes(label.toLowerCase())
    };
  });
}
...
function createBrandCustomTile(title = "", content = "", options = {}) {
  const moduleType = getValidKnowledgeModuleType(options.moduleType);
  return {
...
function isValidKnowledgeModuleType(moduleType = "") {
  return Boolean(getRuntimeKnowledgeModuleDefinition(moduleType));
}

function getValidKnowledgeModuleType(moduleType = "") {
  const definition = getRuntimeKnowledgeModuleDefinition(moduleType);
  return definition?.id || "";
}

function getCustomTileModuleType(tile) {
  return tile && typeof tile === "object" ? getValidKnowledgeModuleType(tile.moduleType) : "";
}
...
function getCanonicalMissingKnowledgeModuleDefinition(value = "") {
  const normalized = normalizeBrandWorkspaceKnowledgeTitle(value);
  return getMissingKnowledgeModuleDefinitions()
    .find((definition) => (
      definition?.id === value
      || normalizeBrandWorkspaceKnowledgeTitle(definition?.label) === normalized
    )) || null;
}

function getCanonicalMissingKnowledgeTitle(value = "") {
  return getCanonicalMissingKnowledgeModuleDefinition(value)?.label || "";
}

function getBrandWorkspaceSectionForCustomTile(tile) {
  const moduleType = getCustomTileModuleType(tile);
  const typedDefinition = moduleType ? getRuntimeKnowledgeModuleDefinition(moduleType) : null;
  if (typedDefinition?.section && BRAND_WORKSPACE_MISSING_KNOWLEDGE_MODULE_IDS.includes(typedDefinition.id)) return typedDefinition.section;
  return getCanonicalMissingKnowledgeModuleDefinition(tile?.title || "")?.section || "";
}

function findBrandWorkspaceCustomTileIndexByModuleDefinition(definition) {
  if (!definition?.id || !Array.isArray(state.brandCore?.customTiles)) return -1;
  const typedIndex = state.brandCore.customTiles.findIndex((tile) => (
    isValidBrandCustomTile(tile)
    && !isMalformedGeneratedCustomTile(tile)
    && getCustomTileModuleType(tile) === definition.id
  ));
  if (typedIndex >= 0) return typedIndex;

  const normalizedTitle = normalizeBrandWorkspaceKnowledgeTitle(definition.label);
  return state.brandCore.customTiles.findIndex((tile) => (
    isValidBrandCustomTile(tile)
    && !isMalformedGeneratedCustomTile(tile)
    && !getCustomTileModuleType(tile)
    && normalizeBrandWorkspaceKnowledgeTitle(tile?.title) === normalizedTitle
  ));
}

function findBrandWorkspaceCustomTileIndexByTitle(value = "") {
  return findBrandWorkspaceCustomTileIndexByModuleDefinition(getCanonicalMissingKnowledgeModuleDefinition(value));
}
...
function createOrSelectMissingKnowledgeTile(rawTitle = "") {
  const definition = getCanonicalMissingKnowledgeModuleDefinition(rawTitle);
  if (!definition?.id || !definition?.label) return;
  state.brandCore = normalizeBrandCoreState(state.brandCore || defaultBrandCoreState());
  let tileIndex = findBrandWorkspaceCustomTileIndexByModuleDefinition(definition);
  if (tileIndex < 0) {
    state.brandCore.customTiles.push(createBrandCustomTile(definition.label, "", { moduleType: definition.id }));
