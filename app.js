...
const el = {
  ...
  brandWorkspaceDetailClose: document.getElementById("brand-workspace-detail-close"),
  ...
  boardBrandCoreCompareOpen: document.getElementById("board-brand-core-compare-open"),
  boardBrandCoreComparison: document.getElementById("board-brand-core-comparison"),
  boardBrandCoreComparisonTitle: document.getElementById("board-brand-core-comparison-title"),
  boardBrandCoreComparisonClose: document.getElementById("board-brand-core-comparison-close"),
  boardBrandCoreComparisonStatus: document.getElementById("board-brand-core-comparison-status"),
  boardBrandCoreComparisonContent: document.getElementById("board-brand-core-comparison-content"),
  boardBrandCoreComparisonRetry: document.getElementById("board-brand-core-comparison-retry"),
  boardBrandCoreComparisonUnsaved: document.getElementById("board-brand-core-comparison-unsaved"),
  boardBrandCoreInitialization: document.getElementById("board-brand-core-initialization"),
  boardBrandCoreInitializeOpen: document.getElementById("board-brand-core-initialize-open"),
  boardBrandCoreInitializationConfirm: document.getElementById("board-brand-core-initialization-confirm"),
  boardBrandCoreInitializationDescription: document.getElementById("board-brand-core-initialization-description"),
  boardBrandCoreInitializationSubmit: document.getElementById("board-brand-core-initialization-confirm-submit"),
...
function readableBrandCoreValue(value) {
  ...
  if (typeof value === "object") return Object.keys(value).length ? JSON.stringify(value, null, 2) : "Not provided";
  return String(value);
}

function brandDetailFieldLabel(key) {
...
function renderCanonicalBrandDetail() {
  ...
  if (!entries.length) {
    ...
    const list = document.createElement("dl"); list.className = "brand-workspace-core-list";
    entries.forEach(([key, value]) => {
      const item = document.createElement("div"); item.className = "brand-workspace-core-item";
      const term = document.createElement("dt"); term.textContent = brandDetailFieldLabel(key);
      const description = document.createElement("dd"); description.textContent = readableBrandCoreValue(value);
...
function canonicalBrandDetailErrorStatus(status) {
  ...
  if ([403, 404].includes(status)) return "unavailable";
  return "error";
}
...
async function loadCanonicalBrandDetail() {
  ...
  try {
    ...
    if (error?.name === "AbortError" || canonicalBrandDetail.requestId !== requestId) return;
    canonicalBrandDetail.status = "error";
    canonicalBrandDetail.controller = null;
    renderCanonicalBrandDetail();
  }
}
...
function resetCanonicalBrandCreation({ focusTrigger = false } = {}) {
  state.brandCreation = { status: "initial", requestId: state.brandCreation.requestId + 1, userEmail: "" };
  el.brandSwitcherCreateForm?.reset();
  el.brandSwitcherCreateForm?.classList.add("hidden");
  el.brandSwitcherCreateOpen?.setAttribute("aria-expanded", "false");
  el.brandSwitcherCreateName?.removeAttribute("aria-invalid");
  if (el.brandSwitcherCreateFeedback) el.brandSwitcherCreateFeedback.textContent = "";
  if (el.brandSwitcherCreateSubmit) el.brandSwitcherCreateSubmit.disabled = false;
  if (el.brandSwitcherCreateCancel) el.brandSwitcherCreateCancel.disabled = false;
  if (focusTrigger && el.brandSwitcherDetails?.open) el.brandSwitcherCreateOpen?.focus();
}

function openCanonicalBrandCreation() {
  if (!state.user?.email || !el.brandSwitcherCreateForm) return;
  resetCanonicalBrandCreation();
  state.brandCreation.status = "editing";
  state.brandCreation.userEmail = state.user.email.trim().toLowerCase();
  el.brandSwitcherCreateForm.classList.remove("hidden");
  el.brandSwitcherCreateOpen?.setAttribute("aria-expanded", "true");
  el.brandSwitcherCreateName?.focus();
}
...
async function submitCanonicalBrandCreation(event) {
  event.preventDefault();
  if (state.brandCreation.status === "submitting") return;
  const userEmail = typeof state.user?.email === "string" ? state.user.email.trim().toLowerCase() : "";
  const name = el.brandSwitcherCreateName?.value.trim() || "";
  if (!name || name.length > 160) {
    state.brandCreation.status = "validation";
    if (el.brandSwitcherCreateFeedback) el.brandSwitcherCreateFeedback.textContent = name ? "Brand name must be 160 characters or fewer." : "Enter a Brand name.";
    el.brandSwitcherCreateName?.setAttribute("aria-invalid", "true");
    el.brandSwitcherCreateName?.focus();
    return;
  }
  if (!userEmail) {
  ...
  const requestId = state.brandCreation.requestId + 1;
  state.brandCreation = { status: "submitting", requestId, userEmail };
  el.brandSwitcherCreateName.value = name;
  el.brandSwitcherCreateName.removeAttribute("aria-invalid");
  el.brandSwitcherCreateSubmit.disabled = true;
  el.brandSwitcherCreateCancel.disabled = true;
  el.brandSwitcherCreateSubmit.textContent = "Creating…";
  if (el.brandSwitcherCreateFeedback) el.brandSwitcherCreateFeedback.textContent = "Creating Canonical Brand…";
  try {
    const response = await fetch("/api/brands", {
      method: "POST",
      headers: { Accept: "application/json", "Content-Type": "application/json" },
      body: JSON.stringify({ name, brand_core: {} })
    });
    if (requestId !== state.brandCreation.requestId || userEmail !== (state.user?.email || "").trim().toLowerCase()) return;
    if (!response.ok) {
      state.brandCreation.status = response.status === 401 ? "unauthenticated" : "error";
      if (response.status === 401) {
        state.user = null;
        state.brandCatalog = { status: "unauthenticated", entries: [], requestId: state.brandCatalog.requestId + 1, userEmail: "" };
        clearEphemeralBrandSwitcherSelection();
      ...
      if (el.brandSwitcherCreateFeedback) el.brandSwitcherCreateFeedback.textContent = canonicalBrandCreationError(response.status);
      return;
    }
    let brand = null;
    try { brand = await response.json(); } catch (_error) { /* handled below */ }
    if (requestId !== state.brandCreation.requestId || userEmail !== (state.user?.email || "").trim().toLowerCase()) return;
    if (!isCanonicalBrandSummary(brand)) {
      state.brandCreation.status = "malformed";
      if (el.brandSwitcherCreateFeedback) el.brandSwitcherCreateFeedback.textContent = "The server returned an unexpected response. The catalog is being refreshed before you take another action.";
    ...
    if (requestId !== state.brandCreation.requestId) return;
    state.brandCreation.status = "error";
    if (el.brandSwitcherCreateFeedback) el.brandSwitcherCreateFeedback.textContent = "The Brand could not be created because the network or server is unavailable. Your name is preserved; retry deliberately.";
  } finally {
    if (requestId === state.brandCreation.requestId && state.brandCreation.status !== "initial") {
      el.brandSwitcherCreateSubmit.disabled = false;
      el.brandSwitcherCreateCancel.disabled = false;
      el.brandSwitcherCreateSubmit.textContent = "Create Brand";
    }
  }
}
...
function compareBrandCoreDocuments(canonical, board) {
  const result = { canonicalOnly: [], boardOnly: [], different: [] };
  const visit = (left, right, path) => {
    ...
    if (Array.isArray(left) && Array.isArray(right)) {
      ...
      for (let index = 0; index < length; index += 1) {
        ...
        if (index >= right.length) result.canonicalOnly.push({ path: nextPath, value: left[index] });
        else if (index >= left.length) result.boardOnly.push({ path: nextPath, value: right[index] });
    ...
    if (isPlainJsonObject(left) && isPlainJsonObject(right)) {
      ...
      keys.forEach((key) => {
        ...
        if (!Object.prototype.hasOwnProperty.call(right, key)) result.canonicalOnly.push({ path: nextPath, value: left[key] });
        else if (!Object.prototype.hasOwnProperty.call(left, key)) result.boardOnly.push({ path: nextPath, value: right[key] });
        else visit(left[key], right[key], nextPath);
      });
      return;
    }
    result.different.push({ path, canonical: left, board: right });
  };
  visit(canonical, board, "$");
  const canonicalEmpty = Object.keys(canonical).length === 0;
  const boardEmpty = Object.keys(board).length === 0;
  return { ...result, matches: !result.canonicalOnly.length && !result.boardOnly.length && !result.different.length, canonicalEmpty, boardEmpty };
}
window.compareBrandCoreDocuments = compareBrandCoreDocuments;

function canonicalInitializationEligible(comparison = boardBrandCoreComparison) {
  const association = state.boardBrandAssociation;
  const snapshot = state.authoritativeBoardBrandCore;
  const userEmail = (state.user?.email || "").trim().toLowerCase();
  return comparison.status === "ready" && !comparison.requiresReload && isPlainJsonObject(comparison.canonical?.brand_core)
    && Object.keys(comparison.canonical.brand_core).length === 0 && Number.isSafeInteger(comparison.canonical.revision) && comparison.canonical.revision >= 1
    && isPlainJsonObject(comparison.boardSnapshot) && Object.keys(comparison.boardSnapshot).length > 0
    && comparison.result?.canonicalEmpty === true && comparison.result?.boardEmpty === false
    && userEmail && comparison.userEmail === userEmail && comparison.boardId === (getBoardIdFromPath() || state.currentBoardId || "")
    && association.boardId === comparison.boardId && association.brandId === comparison.brandId
    && snapshot.boardId === comparison.boardId && snapshot.loadGeneration === comparison.boardLoadGeneration
...
function appendComparisonValue(parent, label, value) {
  const textValue = comparisonValueText(value);
  const wrapper = document.createElement("div"); wrapper.className = "brand-core-comparison-value";
  if (textValue.length <= BRAND_CORE_VALUE_PREVIEW_LIMIT) wrapper.textContent = `${label}: ${textValue}`;
  else {
    const details = document.createElement("details");
    const summary = document.createElement("summary"); summary.textContent = `${label}: ${textValue.slice(0, BRAND_CORE_VALUE_PREVIEW_LIMIT)}… Reveal complete value`;
    const pre = document.createElement("pre"); pre.textContent = textValue;
    details.append(summary, pre); wrapper.appendChild(details);
  }
...
function renderBoardBrandCoreComparison() {
  ...
  try {
    ...
    const eligible = canonicalInitializationEligible(comparison);
    el.boardBrandCoreInitialization?.classList.toggle("hidden", !(eligible || comparison.confirming || comparison.initializationMessage || comparison.requiresReload));
    el.boardBrandCoreInitializeOpen?.classList.toggle("hidden", !eligible || comparison.confirming);
    el.boardBrandCoreInitializationConfirm?.classList.toggle("hidden", !comparison.confirming);
    if (el.boardBrandCoreInitializationDescription) el.boardBrandCoreInitializationDescription.textContent = `Copy this saved Board Brand Core from “${comparison.boardName}” into the empty Canonical Brand Core for “${comparison.brandName}”?`;
    if (el.boardBrandCoreInitializationFeedback) el.boardBrandCoreInitializationFeedback.textContent = comparison.initializationMessage || (comparison.status === "initializing" ? "Initializing Canonical Brand Core…" : "");
...
function beginCanonicalBrandCoreInitialization() {
  const comparison = boardBrandCoreComparison;
  if (!canonicalInitializationEligible(comparison) || comparison.initializationController) return;
  comparison.confirming = true;
  comparison.initializationMessage = "";
  renderBoardBrandCoreComparison();
  el.boardBrandCoreInitializationConfirm?.focus();
}

function cancelCanonicalBrandCoreInitialization() {
  const comparison = boardBrandCoreComparison;
  if (!comparison.confirming || comparison.initializationController) return;
  comparison.confirming = false;
  comparison.initializationId += 1;
  comparison.initializationMessage = "Initialization cancelled. No request was sent.";
  renderBoardBrandCoreComparison();
  el.boardBrandCoreInitializeOpen?.focus();
}

function initializationHttpMessage(status) {
  if (status === 401) return "Your session expired. Sign in again; initialization was not confirmed.";
  if ([403, 404].includes(status)) return "The Canonical Brand is no longer available to this account.";
  if ([400, 422].includes(status)) return "The initialization values were rejected. Reload the authoritative Canonical Brand before trying again.";
  return "The server did not confirm initialization. Reload the authoritative Canonical Brand before trying again.";
}

async function submitCanonicalBrandCoreInitialization() {
  const comparison = boardBrandCoreComparison;
  if (!comparison.confirming || comparison.initializationController || !canonicalInitializationEligible(comparison)) {
    comparison.confirming = false;
    comparison.initializationMessage = "Initialization is no longer available because the comparison context changed.";
    renderBoardBrandCoreComparison();
    return;
  }
  const canonical = comparison.canonical;
  const submittedCore = JSON.parse(JSON.stringify(comparison.boardSnapshot));
  const requestId = comparison.requestId;
  const initializationId = comparison.initializationId + 1;
  const controller = new AbortController();
  comparison.initializationId = initializationId;
  comparison.initializationController = controller;
  comparison.status = "initializing";
  comparison.initializationMessage = "Initializing Canonical Brand Core…";
  renderBoardBrandCoreComparison();
  try {
    const response = await fetch(`/api/brands/${encodeURIComponent(comparison.brandId)}`, {
      method: "PUT", headers: { Accept: "application/json", "Content-Type": "application/json" },
      body: JSON.stringify({ name: canonical.name, brand_core: submittedCore, revision: canonical.revision }), signal: controller.signal
    });
    const current = boardBrandCoreComparison;
    const stillCurrent = current === comparison && current.requestId === requestId && current.initializationId === initializationId
      && current.initializationController === controller && current.userEmail === (state.user?.email || "").trim().toLowerCase()
      && current.boardId === (getBoardIdFromPath() || state.currentBoardId || "") && current.brandId === state.boardBrandAssociation.brandId
      && current.boardLoadGeneration === state.boardLoadGeneration && current.boardLoadGeneration === state.authoritativeBoardBrandCore.loadGeneration
      && state.authoritativeBoardBrandCore.boardId === current.boardId && isPlainJsonObject(state.authoritativeBoardBrandCore.value)
      && JSON.stringify(canonicalJson(state.authoritativeBoardBrandCore.value)) === JSON.stringify(canonicalJson(current.boardSnapshot))
      && state.brandCatalog.status === "success" && state.brandCatalog.userEmail === current.userEmail
      && state.brandCatalog.entries.some(({ id }) => id === current.brandId)
      && el.boardBrandCoreComparison?.open;
    if (!stillCurrent) return;
    current.initializationController = null;
    current.confirming = false;
    if (response.status === 409) {
      current.status = "ready"; current.requiresReload = true;
      current.initializationMessage = "The Canonical Brand changed before initialization could be saved.";
      renderBoardBrandCoreComparison(); el.boardBrandCoreInitializationReload?.focus(); return;
    }
    if (!response.ok) {
      current.status = response.status === 401 ? "unauthenticated" : "ready"; current.requiresReload = true;
      current.initializationMessage = initializationHttpMessage(response.status);
      renderBoardBrandCoreComparison(); return;
    }
    let brand = null; try { brand = await response.json(); } catch (_error) { /* verified below */ }
    const confirmed = brand && brand.id === current.brandId && brand.name === canonical.name
      && brand.revision === canonical.revision + 1 && isPlainJsonObject(brand.brand_core)
      && JSON.stringify(canonicalJson(brand.brand_core)) === JSON.stringify(canonicalJson(submittedCore));
    if (!confirmed || current.requestId !== requestId || current.initializationId !== initializationId) {
      current.status = "ready"; current.requiresReload = true;
      current.initializationMessage = "The server response could not be verified. Reload the authoritative Canonical Brand before trying again.";
      renderBoardBrandCoreComparison(); return;
    }
    current.canonical = (({ id, name, brand_core, revision, updated_at }) => ({ id, name, brand_core: JSON.parse(JSON.stringify(brand_core)), revision, updated_at }))(brand);
    current.result = compareBrandCoreDocuments(current.canonical.brand_core, current.boardSnapshot);
    current.status = "ready"; current.requiresReload = false;
    current.initializationMessage = "Canonical Brand Core initialized from this Board.";
    const entryIndex = state.brandCatalog.entries.findIndex(({ id }) => id === brand.id);
    if (entryIndex >= 0) state.brandCatalog.entries.splice(entryIndex, 1, { ...state.brandCatalog.entries[entryIndex], revision: brand.revision, updated_at: brand.updated_at });
    renderBrandCatalog(); renderBoardBrandCoreComparison(); el.boardBrandCoreComparisonClose?.focus();
  } catch (error) {
    if (error?.name === "AbortError" || boardBrandCoreComparison !== comparison || comparison.initializationId !== initializationId) return;
    comparison.initializationController = null; comparison.confirming = false; comparison.status = "ready"; comparison.requiresReload = true;
    comparison.initializationMessage = "The initialization outcome was not confirmed. Reload the authoritative Canonical Brand before another attempt.";
    renderBoardBrandCoreComparison();
  }
}
...
async function loadBoardBrandCoreComparison() {
  ...
  try {
    ...
    if (!brand || brand.id !== brandId || !isPlainJsonObject(brand.brand_core)) current.status = "malformed";
      ...
      try {
        current.canonical = (({ id, name, brand_core, revision, updated_at }) => ({ id, name, brand_core: JSON.parse(JSON.stringify(brand_core)), revision, updated_at }))(brand);
        current.brandName = typeof brand.name === "string" && brand.name ? brand.name : current.brandName;
        current.result = compareBrandCoreDocuments(current.canonical.brand_core, current.boardSnapshot); current.status = "ready";
      }
...
function renderBoardBrandAssociation() {
  ...
  const canCompare = hasBoard && hasAssociation && !!state.user?.email && catalog.status === "success" && !!catalogBrand
    && catalog.userEmail === (state.user.email || "").trim().toLowerCase() && !state.isBoardLoading
...
function renderAuthState() {
  ...
  if (el.brandSwitcherCreateOpen) el.brandSwitcherCreateOpen.disabled = !signedIn;
...
function renderCampaignIntelligence() {
  let a;
  try {
    a = analyzeCampaign(state.nodes, state.edges, state.brandCore);
  } catch (_error) {
    state.insightsDiagnosticSnapshot = state.currentBoardId && state.boardAccess?.canView !== false
      ? { identity: currentInsightsIdentity(), analysis: null }
      : null;
...
async function requestAiBrainNodeProposal(turnId) {
  ...
  try {
    ...
    const contentType = response.headers?.get?.("content-type") || ""; const isJson = /application\/json/i.test(contentType); const data = isJson ? await response.json().catch(() => null) : null; const current = state.aiBrain.messages.find((item) => item.id === turnId)?.proposal;
    ...
    if (!response.ok) { const failure = new Error("proposal failed"); failure.code = AI_BRAIN_PROPOSAL_ERROR_CODES.has(data?.code) ? data.code : ([404, 405].includes(response.status) || !isJson ? "proposal_route_unavailable" : "proposal_generic_failed"); throw failure; }
    if (!data || data.action !== "propose_content_node" || data.context?.board_id !== boardId || data.context?.response_language !== uiLanguage || data.context?.content_language !== contentLanguage) { const failure = new Error("invalid response"); failure.code = "proposal_response_invalid"; throw failure; }
    const proposal = validateAiBrainNodeProposal(data.node_proposal, turn.id); if (!proposal || state.aiBrain.appliedProposalIds.has(proposal.proposal_id)) { const failure = new Error("invalid proposal"); failure.code = "proposal_response_invalid"; throw failure; }
    ...
    setTurnProposal(turnId, () => ({ status: "failed", errorCode: error?.code || "proposal_network_failed", uiLanguage })); renderAiBrain();
  }
}

// Model output is untrusted. This intentionally small formatter creates only
// semantic, non-interactive elements and assigns every model string via textContent.
function appendAiBrainInline(parent, value) {
  const text = typeof value === "string" ? value : "";
  const token = /(\*\*([^*\n]+)\*\*|`([^`\n]+)`)/g;
  let cursor = 0; let match;
  const appendText = (content) => { if (!content) return; const span = document.createElement("span"); span.textContent = content; parent.appendChild(span); };
  while ((match = token.exec(text))) {
    appendText(text.slice(cursor, match.index));
    const element = document.createElement(match[2] !== undefined ? "strong" : "code");
    element.textContent = match[2] !== undefined ? match[2] : match[3];
    parent.appendChild(element); cursor = match.index + match[0].length;
  }
  appendText(text.slice(cursor));
}
function appendAiBrainLineBreaks(parent, value) {
  String(value).split("\n").forEach((line, index) => { if (index) parent.appendChild(document.createElement("br")); appendAiBrainInline(parent, line); });
}
function renderAiBrainFormattedAnswer(container, answer) {
  const lines = (typeof answer === "string" ? answer : "").replace(/\r\n?/g, "\n").split("\n");
  let index = 0;
  while (index < lines.length) {
    if (!lines[index].trim()) { index += 1; continue; }
    const heading = lines[index].match(/^ {0,3}(#{1,4})[ \t]+(.+?)\s*$/);
    if (heading) {
      const element = document.createElement(`h${Math.min(heading[1].length + 2, 6)}`); appendAiBrainInline(element, heading[2]); container.appendChild(element); index += 1; continue;
    }
    const unordered = lines[index].match(/^ {0,3}[-*+]\s+(.+)$/);
    const ordered = lines[index].match(/^ {0,3}\d{1,3}\.\s+(.+)$/);
    if (unordered || ordered) {
      const list = document.createElement(unordered ? "ul" : "ol");
      const pattern = unordered ? /^ {0,3}[-*+]\s+(.+)$/ : /^ {0,3}\d{1,3}\.\s+(.+)$/;
      while (index < lines.length) {
        const itemMatch = lines[index].match(pattern); if (!itemMatch) break;
        const item = document.createElement("li"); appendAiBrainInline(item, itemMatch[1]); list.appendChild(item); index += 1;
      }
      container.appendChild(list); continue;
    }
    const paragraphLines = [];
    while (index < lines.length && lines[index].trim()
      && !/^ {0,3}#{1,4}[ \t]+.+/.test(lines[index])
      && !/^ {0,3}(?:[-*+]\s+|\d{1,3}\.\s+).+/.test(lines[index])) paragraphLines.push(lines[index++]);
    const paragraph = document.createElement("p"); appendAiBrainLineBreaks(paragraph, paragraphLines.join("\n")); container.appendChild(paragraph);
  }
}
...
function createAiBrainAdvisorAvatar(identity = captureAiBrainAdvisorIdentity()) {
  const shell = document.createElement("span"); shell.className = `ai-brain-advisor-avatar is-${identity?.fallback || "funklix-brain"}`;
  shell.setAttribute("role", "img"); shell.setAttribute("aria-label", uiText("Brand advisor avatar"));
  const fallback = () => { shell.textContent = "✦"; shell.classList.add("is-fallback"); };
  if (identity?.avatarSource) { const image = document.createElement("img"); image.alt = ""; image.src = identity.avatarSource; image.addEventListener("error", () => { image.remove(); fallback(); }, { once: true }); shell.append(image); } else fallback();
  return shell;
}
function renderAiBrainTranscript(transcript) {
  transcript.textContent = "";
  if (!state.aiBrain.messages.length) {
    const empty = document.createElement("div"); empty.className = "ai-brain-empty";
    const heading = document.createElement("h4"); heading.textContent = uiText("Ask your Brand advisor about this campaign.");
    const copy = document.createElement("p"); copy.textContent = uiText("I can explain Canvas diagnostics and advise without changing it.");
    empty.append(createAiBrainAdvisorAvatar(), heading, copy); transcript.appendChild(empty); return;
  }
  state.aiBrain.messages.forEach((turn) => {
    const user = document.createElement("article"); user.className = "ai-brain-message is-user"; user.dataset.turnId = turn.id;
    const userLabel = document.createElement("strong"); userLabel.textContent = aiBrainText("you");
    const question = document.createElement("div"); question.className = "ai-brain-question"; question.textContent = turn.question;
    ...
    if (Array.isArray(turn.assumptions) && turn.assumptions.length) {
      ...
      const list = document.createElement("ul"); turn.assumptions.forEach((value) => { const item = document.createElement("li"); item.textContent = typeof value === "string" ? value : String(value); list.appendChild(item); });
...
async function requestAiBrainAdvice(event, retryTurnId = null) {
  ...
  const responseLanguage = state.uiLanguage === "de" ? "de" : "en"; const controller = new AbortController(); const requestId = state.aiBrain.requestId + 1; const advisorIdentity = captureAiBrainAdvisorIdentity(requestId); const identity = aiBrainIdentity(); const contextIdentity = aiBrainRequestContextIdentity(); const selectedNodeId = state.selectedPrimary || null; const canvas = aiBrainCanvasProjection(); const conversationHistory = aiBrainConversationHistory(existing?.id || null);
  const turn = existing || { id: `turn-${requestId}`, question }; const messages = existing ? state.aiBrain.messages.map((item) => item.id === turn.id ? { ...item, status: "pending", responseLanguage, advisorIdentity, errorCode: "", answer: "" } : item) : [...state.aiBrain.messages, { ...turn, status: "pending", responseLanguage, advisorIdentity }];
...
function buildInsightsFindings(analysis) {
  ...
  const bounded = (values) => Object.freeze([...new Set((values || []).filter((value) => typeof value === "string" && value.trim()).map((value) => value.trim().slice(0, 160)))].slice(0, 8));
  const add = (finding) => findings.push(Object.freeze({ ...finding, evidence: bounded(finding.evidence), currentValues: bounded(finding.currentValues), missingElements: bounded(finding.missingElements), affectedNodeIds: Object.freeze(ids(finding.nodes || [])) }));
  ...
  if (missingCta.length) add({ code: "CTA_MISSING", title: "CTA structure needs attention", description: "One or more eligible Canvas nodes do not contain a clear CTA.", severity: "important", category: "content", evidence: [`${missingCta.length} campaign item(s) have no clear next step.`], missingElements: ["Clear call to action"], structuralRelevance: 3, weight: 3, nodes: missingCta });
  if (analysis.funnel.missingStages.length) add({ code: "FUNNEL_STAGE_GAPS", title: "Funnel-stage gaps remain", description: "One or more campaign stages are not represented in the current Canvas.", severity: analysis.funnel.missingStages.length >= 3 ? "important" : "opportunity", category: "funnel", evidence: [`Covered stages: ${analysis.funnel.coveredStages.join(", ") || "none"}.`], currentValues: analysis.funnel.coveredStages, missingElements: analysis.funnel.missingStages, structuralRelevance: 4, weight: analysis.funnel.missingStages.length, nodes: [] });
  const landingMissingTrust = state.nodes.filter((node) => node.type === "Landing Page" && !(node.landingPage?.trust || "").trim());
  if (analysis.trust.suggestions.length) add({ code: "TRUST_PROOF_MISSING", title: "Trust-layer coverage is incomplete", description: "Landing Page trust evidence is absent from the current Canvas.", severity: "opportunity", category: "strategy", evidence: ["A Landing Page has no trust-building proof."], missingElements: ["Trust evidence"], structuralRelevance: 2, weight: 2, nodes: landingMissingTrust });
  const audienceNodes = state.nodes.filter((node) => (node.audience || "").trim());
  if (analysis.icp.inconsistencies.length) add({ code: "ICP_VARIATION", title: "ICP consistency needs review", description: "Several audience descriptions differ across the Canvas. Aligning them would make the campaign more focused.", severity: "opportunity", category: "strategy", evidence: analysis.icp.inconsistencies, currentValues: audienceNodes.map((node) => node.audience), structuralRelevance: 2, weight: 2, nodes: audienceNodes });
  const toneNodes = state.nodes.filter((node) => (node.tone || "").trim());
  if (analysis.tone.warnings.length) add({ code: "TONE_SHIFTS_HIGH", title: "Tone consistency needs review", description: "Most campaign content follows a similar tone, with a few noticeable variations.", severity: "opportunity", category: "strategy", evidence: analysis.tone.warnings, currentValues: toneNodes.map((node) => node.tone), structuralRelevance: 1, weight: 1, nodes: toneNodes });
  if (analysis.cta.suggestions.length && !missingCta.length) add({ code: "CTA_VARIATIONS_MISSING", title: "CTA variation is limited", description: "Calls to action are present, but they offer limited variation across the campaign.", severity: "opportunity", category: "content", evidence: analysis.cta.suggestions, currentValues: analysis.cta.values, missingElements: ["CTA variation"], structuralRelevance: 2, weight: 1, nodes: ctaNodes });
...
function askAiBrainAboutFinding(code) {
  ...
  const affectedNodes = finding.affectedNodeIds.map((id) => state.nodes.find((node) => node.id === id)).filter(Boolean).slice(0, 3);
  if (affectedNodes.length !== Math.min(finding.affectedNodeIds.length, 3) || !Array.isArray(finding.evidence)) return setSaveStatus(uiText("This insight is no longer current. Refresh AI Insights."));
  const descriptor = Object.freeze({ findingId: finding.code, title: uiText(finding.title).slice(0, 160), explanation: uiText(finding.description).slice(0, 500), severity: finding.severity, category: finding.category, evidence: Object.freeze(finding.evidence.map(insightsDiagnosticText).slice(0, 8)), affectedNodes: Object.freeze(affectedNodes.map((node) => Object.freeze({ id: node.id, title: String(node.title || node.type || uiText("Node")).slice(0, 160), role: String(node.type || uiText("Node")).slice(0, 80) }))), currentValues: Object.freeze((finding.currentValues || []).slice(0, 8)), missingElements: Object.freeze((finding.missingElements || []).slice(0, 8)), canvasState: snapshot.canvasState, analyzedAt: snapshot.analyzedAt });
  const join = (values) => values.filter(Boolean).join(", ");
  const nodeText = descriptor.affectedNodes.map((node) => `“${node.title}” — ${node.role}`).join("; ");
  const context = [descriptor.currentValues.length ? `${state.uiLanguage === "de" ? "Vorhanden" : "Present"}: ${join(descriptor.currentValues)}` : "", descriptor.missingElements.length ? `${state.uiLanguage === "de" ? "Fehlend oder uneinheitlich" : "Missing or inconsistent"}: ${join(descriptor.missingElements)}` : ""].filter(Boolean).join(". ") || descriptor.explanation;
  const request = descriptor.findingId.startsWith("CTA_") ? (state.uiLanguage === "de" ? "schlage eine konkrete verbesserte CTA vor" : "suggest a concrete improved CTA") : descriptor.findingId === "FUNNEL_STAGE_GAPS" ? (state.uiLanguage === "de" ? "schlage das nützlichste Content- oder Node-Konzept für die fehlende Funnel-Stufe vor" : "suggest the most useful content or node concept for the missing funnel stage") : descriptor.findingId === "ICP_VARIATION" ? (state.uiLanguage === "de" ? "benenne die widersprüchlichen Zielgruppenbeschreibungen und schlage eine konsistente Richtung vor" : "identify the conflicting audience descriptions and suggest a consistent direction") : descriptor.findingId === "TONE_SHIFTS_HIGH" ? (state.uiLanguage === "de" ? "benenne die relevante Abweichung und schlage eine passende Formulierung vor" : "identify the relevant variation and suggest aligned wording") : descriptor.category === "content" ? (state.uiLanguage === "de" ? "schlage eine relevante Kanal- oder Content-Anpassung ohne erfundene Performance-Aussagen vor" : "suggest a relevant channel or content adaptation without fabricated performance claims") : (state.uiLanguage === "de" ? "erkläre das fehlende Element und empfehle die nächste sinnvolle Ergänzung" : "explain the missing element and recommend the next useful addition");
  const question = state.uiLanguage === "de" ? `Hilf mir, diesen Canvas-Hinweis zu verbessern:\n\nHinweis: ${descriptor.title}\nErkannt wurde: ${descriptor.evidence.join(" ") || descriptor.explanation}\n${nodeText ? `Betroffene Elemente: ${nodeText}\n` : ""}Aktueller Kontext: ${context}\nCanvas-Stand: ${descriptor.canvasState === "live_unsaved" ? "Canvas mit ungespeicherten Änderungen" : "Aktuell gespeicherter Canvas"}.\nAnalysezeitpunkt: ${descriptor.analyzedAt}\n\nBitte:\n1. erkläre, warum das für die Kampagne relevant ist;\n2. prüfe die betroffenen Elemente anhand des aktuell autorisierten Canvas;\n3. ${request};\n4. halte die Empfehlung konsistent mit Brand Core und Kampagnenstrategie.` : `Help me improve this Canvas finding:\n\nFinding: ${descriptor.title}\nWhat was detected: ${descriptor.evidence.join(" ") || descriptor.explanation}\n${nodeText ? `Affected items: ${nodeText}\n` : ""}Current context: ${context}\nCanvas state: ${descriptor.canvasState === "live_unsaved" ? "Canvas with unsaved changes" : "Current saved Canvas"}.\nAnalysis time: ${descriptor.analyzedAt}\n\nPlease:\n1. explain why this matters for the campaign;\n2. review the affected items using the current authorized Canvas;\n3. ${request};\n4. keep the recommendation consistent with the Brand Core and campaign strategy.`;
...
function renderInsightsSurface() {
  ...
  // Build off-DOM so an exception cannot expose a half-rendered surface.
  const root = make("div", "insights-boundary");
  ...
  const unavailableReason = state.boardAccess?.canView === false ? "Canvas diagnostics are unavailable for this Board." : !state.currentBoardId ? "No Board is open. Open an authorized Board to view Canvas diagnostics." : state.boardLoadState === "loading" ? "Board is loading. Canvas diagnostics will appear when the authorized Canvas is ready." : !state.nodes.length ? "This Canvas is empty. Add campaign content to make deterministic diagnostics available." : !snapshot?.analysis || !isValidInsightsDiagnostic(snapshot.analysis) ? "Canvas diagnostics could not be calculated. No diagnostic result is shown." : "";
  const scoreStatusKey = (score) => score >= 85 ? "Strong" : score >= 70 ? "Good foundation" : score >= 45 ? "Needs attention" : "Incomplete";
  const funnelStages = ["Awareness", "Interest", "Consideration", "Conversion", "Retention"];
  const section = (id, title, description, extra = "") => { const node = make("section", `insights-section ${extra}`.trim()); node.setAttribute("aria-labelledby", id); const heading = make("div", "insights-section-heading"); const h = make("h3", "", t(title)); h.id = id; heading.append(h); if (description) heading.append(make("p", "", t(description))); node.append(heading); root.append(node); return node; };
  const header = make("header", "insights-page-header"); const heading = make("div", "insights-header-copy"); heading.append(make("h2", "", t("AI Insights")), make("p", "insights-purpose", t("See what is ready, what needs attention, and where to focus next.")), make("p", "insights-source", t("Based on the structure and content of your current Canvas.")));
  const context = make("div", "insights-context"); [state.currentBoardName || t("Current Board"), `${state.nodes.length} ${t("Canvas nodes")}`, sourceState, snapshot?.analyzedAt ? `${t("Last analyzed")}: ${new Intl.DateTimeFormat(state.uiLanguage === "de" ? "de-DE" : "en", { dateStyle: "medium", timeStyle: "short" }).format(new Date(snapshot.analyzedAt))}` : ""].filter(Boolean).forEach((value) => context.append(make("span", "", value)));
  const refresh = make("button", "fk-btn fk-btn-secondary insights-refresh", t("Refresh insights")); refresh.type = "button"; refresh.addEventListener("click", renderCampaignIntelligence); header.append(heading, context, refresh); root.append(header);
  const overview = section("insights-overview-title", "Executive health overview", "A compact view of current Canvas readiness.", "insights-overview");
  if (unavailableReason) { const status = make("div", "insights-diagnostic-state", t(unavailableReason)); status.setAttribute("role", "status"); overview.append(status); }
  if (!unavailableReason) {
    const a = snapshot.analysis; const panel = make("div", "insights-executive-panel");
    const readiness = make("article", "insights-readiness"); const ring = make("div", "insights-readiness-ring"); ring.style.setProperty("--insights-score", String(a.healthScore)); ring.setAttribute("role", "img"); ring.setAttribute("aria-label", `${t("Campaign readiness")}: ${a.healthScore}/100, ${t(scoreStatusKey(a.healthScore))}`); ring.append(make("strong", "", String(a.healthScore)), make("span", "", "/100")); readiness.append(ring, make("h4", "", t("Campaign readiness")), make("strong", "insights-quality-label", t(scoreStatusKey(a.healthScore))), make("p", "", t("Your campaign structure is mostly complete, with a few areas still worth refining.")));
    const attention = make("article", "insights-attention"); attention.append(make("h4", "", t("Needs attention"))); const attentionItems = (Array.isArray(snapshot.findings) ? snapshot.findings : []).filter((finding) => scoreStatusKey(({ ICP_VARIATION:a.icp.consistencyScore, TONE_SHIFTS_HIGH:a.tone.consistencyScore, TRUST_LAYER_MISSING:a.trust.score, CTA_MISSING:a.cta.qualityScore, CTA_VARIATIONS_MISSING:70, FUNNEL_STAGE_GAPS:a.funnel.confidence })[finding.code] ?? 50) !== "Strong").slice(0, 3);
    if (!attentionItems.length) attention.append(make("p", "insights-attention-empty", t("No areas need attention right now."))); else { const list = make("ul", "insights-attention-list"); attentionItems.forEach((finding) => { const li=make("li"); const button=make("button", "", t(finding.title)); button.type="button"; button.addEventListener("click",()=>root.querySelector(`[data-finding-id='${finding.code}']`)?.scrollIntoView({behavior:"smooth",block:"center"})); li.append(button); list.append(li); }); attention.append(list); }
    const journey = make("article", "insights-journey-summary"); const covered = new Set(a.funnel.coveredStages); journey.append(make("h4", "", t("Journey summary")), make("strong", "", `${covered.size} ${t("stages covered")} · ${5-covered.size} ${t("stages missing")}`)); const preview=make("div","insights-journey-preview"); funnelStages.forEach(stage=>{const dot=make("span",covered.has(stage)?"is-covered":"is-missing",covered.has(stage)?"✓":"!");dot.title=t(stage);dot.setAttribute("aria-label",`${t(stage)}: ${t(covered.has(stage)?"Covered":"Missing content")}`);preview.append(dot);});journey.append(preview);
    const channels=make("article","insights-channels"); const entries=Object.entries(a.platformDistribution.counts).filter(([,count])=>Number.isFinite(count)&&count>0); channels.append(make("h4","",t("Channel distribution")),make("strong","",`${entries.length} ${t(entries.length===1?"channel":"channels")}`)); if(!entries.length) channels.append(make("p","insights-channel-empty",t("No supported channel content is present yet."))); else { const list=make("div","insights-channel-list"); const max=Math.max(...entries.map(([,count])=>count)); entries.forEach(([name,count])=>{const row=make("div","insights-channel-row");const label=make("span","",`${String(name).slice(0,80)} · ${count}`);const bar=make("span","insights-channel-bar");bar.style.setProperty("--channel-share",`${count/max*100}%`);row.append(label,bar);list.append(row);});channels.append(list); }
    panel.append(readiness,attention,journey,channels); overview.append(panel);
  }
  const journeySection=section("insights-journey-title","Campaign journey coverage","See which stages of the campaign journey are supported by content in your Canvas.","insights-journey-section");
  if(!unavailableReason){const covered=new Set(snapshot.analysis.funnel.coveredStages);const list=make("ol","insights-campaign-journey");funnelStages.forEach(stage=>{const present=covered.has(stage);const item=make("li",present?"is-covered":"is-missing");item.setAttribute("aria-label",`${t(stage)}: ${t(present?"Covered":"Missing content")}`);item.append(make("span","insights-stage-icon",present?"✓":"!"),make("strong","",t(stage)),make("span","insights-stage-status",t(present?"Covered":"Missing content")));list.append(item);});journeySection.append(list,make("p","insights-journey-disclaimer",t("Canvas content coverage, not measured conversion performance.")));}
  const opportunities=section("insights-opportunities-title","Opportunities","Focus on the next useful improvements for this Canvas.","insights-opportunities");
  if(!unavailableReason){if(!(Array.isArray(snapshot.findings)&&snapshot.findings.length))opportunities.append(make("p","insights-diagnostic-state",t("No current findings")));(Array.isArray(snapshot.findings)?snapshot.findings:[]).filter(Boolean).forEach(finding=>{const item=make("article",`insights-finding severity-${finding.severity}`);item.dataset.findingId=finding.code;const severity=t(finding.severity==="critical"?"High priority":finding.severity==="important"?"Worth improving":"Minor improvement");const icon=make("span","insights-severity-icon",finding.severity==="critical"?"!":"↗");icon.setAttribute("aria-hidden","true");item.append(icon,make("span","insights-severity",severity),make("h4","",t(finding.title)),make("p","",t(finding.description)));const validIds=(Array.isArray(finding.affectedNodeIds)?finding.affectedNodeIds:[]).filter(id=>state.nodes.some(node=>node.id===id));if(validIds.length){item.append(make("p","insights-affected-count",`${validIds.length} ${t("Affected nodes")}`));const list=make("ul","insights-node-list");validIds.slice(0,3).forEach(id=>{const node=state.nodes.find(entry=>entry.id===id);list.append(make("li","",`${String(node.title||node.type||t("Node")).slice(0,160)} — ${String(node.type||t("Node")).slice(0,80)}`));});if(validIds.length>3)list.append(make("li","",`+${validIds.length-3} ${t("more")}`));item.append(list);}const actions=make("div","insights-actions");if(validIds.length){const show=make("button","fk-btn fk-btn-secondary",t("Show on Canvas"));show.type="button";show.addEventListener("click",()=>showInsightsFindingOnCanvas(finding.code));actions.append(show);}if(state.user?.email&&state.boardAccess?.canEdit===true&&!state.publicBoardToken){const ask=make("button","fk-btn fk-btn-primary",t("Ask AI Brain"));ask.type="button";ask.addEventListener("click",()=>askAiBrainAboutFinding(finding.code));actions.append(ask);}item.append(actions);opportunities.append(item);});}
...
function isValidInsightsDiagnostic(analysis) {
  return !!analysis && Number.isFinite(analysis.healthScore) && Array.isArray(analysis.funnel?.coveredStages)
    && Array.isArray(analysis.funnel?.missingStages) && analysis.platformDistribution?.counts
    && Number.isFinite(analysis.cta?.qualityScore) && Number.isFinite(analysis.icp?.consistencyScore)
    && Number.isFinite(analysis.tone?.consistencyScore) && Number.isFinite(analysis.trust?.score);
}
...
function renderFunnelSimulator() {
  if (!window.FunklixFunnelSimulator || !window.FunklixPersonaJourneySimulator || !el.funnelSimulatorSurface) return;
  ...
  const canvasEdges = state.edges.map((edge) => { const projected = {}; ["id", "source", "target", "type"].forEach((key) => { if (typeof edge[key] === "string") projected[key] = edge[key]; }); return projected; });
  const identity = `${state.user?.email || "anonymous"}|${state.currentBoardId || ""}|${state.boardLoadGeneration}|${state.boardAccess?.canView !== false}|${state.publicBoardToken || ""}`;
  window.FunklixPersonaJourneySimulator.mount(el.funnelSimulatorSurface, {
    language: state.uiLanguage,
    identity,
    boardId: state.currentBoardId || "",
    boardName: state.currentBoardName || uiText("Current Board"),
    ...
    brandCoreRevision: state.authoritativeBoardBrandCore?.provenance?.revision || state.authoritativeBoardBrandCore?.updatedAt || state.lastKnownUpdatedAt || "0",
    nodes: state.nodes,
    canvasContext: { nodes: canvasNodes, edges: canvasEdges },
...
el.boardBrandCoreComparisonRetry?.addEventListener("click", () => { void loadBoardBrandCoreComparison(); });
el.boardBrandCoreInitializeOpen?.addEventListener("click", beginCanonicalBrandCoreInitialization);
...
// BW-32.2.1: account-scoped LinkedIn connection controls and bounded callback recovery.
(function initializeLinkedInSettings(){
  const card=document.getElementById('linkedin-connection-card'),connect=document.getElementById('linkedin-connect-button'),disconnect=document.getElementById('linkedin-disconnect-button'),status=document.getElementById('linkedin-connection-status'),detail=document.getElementById('linkedin-connection-detail'),result=document.getElementById('linkedin-connection-result'),copy=document.getElementById('linkedin-copy-diagnostics'),dialog=document.getElementById('settings-dialog');
  if(!card||!connect)return;let connectionId=null,busy=false,diagnostics='',callbackFeedback=null;
  const outcomes={connected:'LinkedIn was connected successfully.',connected_limited:'LinkedIn is connected. Publishing permission is not available yet.',cancelled:'LinkedIn connection was cancelled.',expired:'The connection request expired. Start the connection again.',configuration_failed:'LinkedIn setup needs attention before an account can be connected.',token_exchange_failed:'LinkedIn could not complete the authorization. Try connecting again.',identity_failed:'LinkedIn authorized the request, but the profile identity could not be verified.',permission_missing:'LinkedIn did not grant the required profile permission. Connect again and approve profile access.',storage_failed:'The account was authorized, but Funklix could not save the connection.',connection_failed:'LinkedIn could not complete the connection. Try connecting again.'};
  const callbackKeys=['social','classification','phase','operation_category','commit_status','attempt_id','client_request_id','server_request_id','timestamp'];
  const params=new URLSearchParams(location.search),returned=params.get('social');
  if(returned&&Object.hasOwn(outcomes,returned)){
    const safeId=value=>/^[A-Za-z0-9_-]{8,128}$/.test(value||'')?value:'';
    const safeWord=value=>/^[a-z_]{2,40}$/.test(value||'')?value:'';
    const values={timestamp:/^\d{4}-\d\d-\d\dT[\d:.]+Z$/.test(params.get('timestamp')||'')?params.get('timestamp'):'',outcome:returned,classification:safeWord(params.get('classification')),phase:safeWord(params.get('phase')),operationCategory:safeWord(params.get('operation_category')),committedState:safeWord(params.get('commit_status')),attemptId:/^[0-9a-f-]{36}$/i.test(params.get('attempt_id')||'')?params.get('attempt_id'):'',clientRequestId:safeId(params.get('client_request_id')),serverRequestId:safeId(params.get('server_request_id'))};
    callbackFeedback={outcome:returned,attemptId:values.attemptId};result.dataset.state=returned==='connected'?'success':returned==='connected_limited'?'warning':'error';result.textContent=language?.t?.(outcomes[returned])||outcomes[returned];diagnostics=Object.entries(values).filter(([,value])=>value).map(([key,value])=>`${key}: ${value}`).join('\n');if(copy){copy.hidden=!diagnostics;copy.addEventListener('click',()=>navigator.clipboard?.writeText(diagnostics));}
    callbackKeys.forEach(key=>params.delete(key));const query=params.toString();history.replaceState(history.state,'',`/${query?`?${query}`:''}${location.hash}`);
    queueMicrotask(()=>{if(dialog&&!dialog.open)dialog.showModal();document.getElementById('social-connections-settings')?.scrollIntoView({block:'nearest'});card.focus();result.focus();});
  }
  async function refresh(){try{const response=await fetch('/api/social-connections',{headers:{accept:'application/json'}});if(!response.ok)return;const data=await response.json(),li=data.linkedin;connectionId=li.connectionId;if(callbackFeedback&&callbackFeedback.attemptId&&callbackFeedback.attemptId===li.attemptId&&li.coherent&&(callbackFeedback.outcome==='storage_failed'||callbackFeedback.outcome==='connected'||callbackFeedback.outcome==='connected_limited')){callbackFeedback.outcome=li.publishingPermission?'connected':'connected_limited';result.dataset.state=li.publishingPermission?'success':'warning';result.textContent=language?.t?.(outcomes[callbackFeedback.outcome])||outcomes[callbackFeedback.outcome];}else if(callbackFeedback&&callbackFeedback.attemptId&&li.attemptId&&callbackFeedback.attemptId!==li.attemptId){result.textContent='';copy.hidden=true;callbackFeedback=null;}if(!li.configured){connect.disabled=true;connect.textContent='Setup required';return;}if(!connectionId||li.status==='disconnected'){status.textContent='Not connected';connect.disabled=false;connect.textContent='Connect LinkedIn';return;}status.textContent=li.displayName||'LinkedIn';detail.textContent=li.status==='needs_attention'?'Reconnect required':`${li.accountType==='personal'?'Personal profile':'LinkedIn profile'} · ${li.publishingPermission?'Publishing permission available':'Limited permissions'}${li.expiresAt?` · Token expires ${new Date(li.expiresAt).toLocaleDateString()}`:''}`;connect.disabled=false;connect.textContent='Reconnect';disconnect.hidden=false;}catch{connect.disabled=true;}}
  connect.addEventListener('click',async()=>{if(busy)return;busy=true;connect.disabled=true;connect.textContent='Opening LinkedIn…';const clientRequestId=`cli_${crypto.randomUUID().replaceAll('-','')}`;try{const response=await fetch('/api/social-connections-linkedin-start',{method:'POST',headers:{'content-type':'application/json','x-client-request-id':clientRequestId},body:JSON.stringify({returnPath:'/'})});const data=await response.json();if(!response.ok||!data.ok)throw new Error();location.assign(data.value.authorizationUrl);}catch{result.textContent='LinkedIn could not complete the connection. Try connecting again.';busy=false;connect.disabled=false;connect.textContent=connectionId?'Reconnect':'Connect LinkedIn';}});
  disconnect.addEventListener('click',async()=>{if(!connectionId||!confirm('Disconnect LinkedIn?'))return;disconnect.disabled=true;try{const response=await fetch('/api/social-connections-linkedin-disconnect',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({connectionId})});if(!response.ok)throw new Error();connectionId=null;disconnect.hidden=true;status.textContent='Not connected';detail.textContent='LinkedIn is disconnected.';connect.textContent='Connect LinkedIn';result.textContent='LinkedIn disconnected.';}catch{result.textContent='LinkedIn could not complete the connection. Try connecting again.';}finally{disconnect.disabled=false;}});
  dialog?.addEventListener('toggle',refresh);refresh();
})();
