...
function renderAiBrainTranscript(transcript) {
  ...
  if (!state.aiBrain.messages.length) {
    ...
    const heading = document.createElement("h4"); heading.textContent = "Ask about your Brand or campaign";
    const copy = document.createElement("p"); copy.textContent = "I can explain Canvas diagnostics and advise without changing it.";
    empty.append(heading, copy); transcript.appendChild(empty); return;
  }
...
function renderInsightsSurface() {
  ...
  const scoreLabel = (score) => t(score >= 85 ? "Strong" : score >= 70 ? "Good foundation" : score >= 45 ? "Needs attention" : "Incomplete");
  const funnelStages = ["Awareness", "Interest", "Consideration", "Conversion", "Retention"];
  const funnelChecklist = (analysis) => { const list = element("ul", "insights-funnel-stages"); const covered = new Set(analysis.funnel.coveredStages); funnelStages.forEach((stage) => { const present = covered.has(stage); const item = element("li", present ? "is-covered" : "is-missing"); const icon = element("span", "insights-stage-icon", present ? "✓" : "○"); icon.setAttribute("aria-hidden", "true"); item.append(icon, element("span", "", `${t(stage)}${present ? ` — ${t("Covered")}` : ` — ${t("Missing")}`}`)); list.append(item); }); return list; };
  const channelList = (analysis) => { const list = element("div", "insights-channel-list"); const entries = Object.entries(analysis.platformDistribution.counts); if (!entries.length) { list.append(element("p", "insights-channel-empty", t("No supported channel content is present yet."))); return list; } entries.forEach(([name, count]) => { const row = element("div", "insights-channel-row"); row.append(element("span", "", name), element("strong", "", String(count))); list.append(row); }); return list; };
  const header = element("header", "insights-page-header"); header.append(element("p", "insights-eyebrow", t("Evidence from your campaign Canvas")), element("p", "insights-lede", t("Based on the structure and content of your current Canvas.")));
  ...
  const overview = section("insights-overview-title", "Overview", "See what is ready and where your campaign needs attention.");
  if (unavailableReason) { const status = element("div", "insights-diagnostic-state", t(unavailableReason)); status.setAttribute("role", "status"); status.append(element("p", "", t(!state.nodes.length && state.currentBoardId ? "Add or generate campaign nodes, then return to AI Insights." : "Your campaign data was not changed. Refresh the insights or return to the Canvas."))); overview.append(status); }
    ...
    const a = snapshot.analysis; const grid = element("div", "insights-grid insights-overview-grid");
    const card = (title, value, label, description, content) => { const item = element("article", "insight-card insights-summary-card"); item.append(element("h4", "", t(title)), element("div", "insight-value", value)); if (label) item.append(element("strong", "insights-quality-label", label)); item.append(element("p", "", t(description))); if (content) item.append(content); return item; };
    grid.append(card("Campaign readiness", `${a.healthScore}/100`, scoreLabel(a.healthScore), "Your campaign structure is mostly complete, with a few areas still worth refining."), card("Areas needing attention", String(snapshot.findings.length), "", snapshot.findings.length ? "Review these areas to make the campaign clearer and more complete." : "No current findings"), card("Funnel coverage", `${a.funnel.coveredStages.length}/5`, scoreLabel(a.funnel.confidence), "See which campaign stages are represented.", funnelChecklist(a)), card("Channel coverage", String(Object.keys(a.platformDistribution.counts).length), "", "Content by supported channel.", channelList(a))); overview.append(grid);
  }
  const measured = section("measured-performance-title", "Measured performance", "No analytics data connected", "insights-measured-empty"); measured.append(element("span", "insights-data-status", t("No analytics data connected")), element("p", "", t("Campaign results such as reach, engagement, conversions, and revenue will appear here when a verified data source is connected.")));
  ...
  if (!unavailableReason) { const a = snapshot.analysis; const groups = [["Structure and readiness", [["Campaign readiness", a.healthScore, "Most essential campaign elements are present. Review the highlighted gaps before launch.", "Canvas readiness formula"]]], ["Funnel coverage", [["Funnel-stage coverage", a.funnel.confidence, "The checklist shows which stages are present and which still need content.", "Funnel-stage mapping"]]], ["Strategy and consistency", [["ICP consistency", a.icp.consistencyScore, "Several audience descriptions may differ across the Canvas. Align them to keep the campaign focused.", "Distinct audience labels"], ["Tone consistency", a.tone.consistencyScore, "Most campaign content follows a similar tone, with any variations highlighted below.", "Distinct tone labels"], ["Trust-layer coverage", a.trust.score, "Trust-building elements help complete the campaign structure.", "Landing Page trust-field presence"]]], ["Content and channels", [["CTA structure", a.cta.qualityScore, "Calls to action are present, but some content may need a clearer next step.", "CTA variation check"], ["Channel coverage", null, "See how campaign content is distributed across supported channels.", "Platform counting"]]]]; const grid = element("div", "insights-diagnostic-groups"); groups.forEach(([name, values]) => { const group = element("section", "insights-diagnostic-group"); group.append(element("h4", "", t(name))); values.forEach(([name, score, interpretation, method]) => { const card = element("article", "insight-card insights-diagnostic-row"); card.append(element("h5", "", t(name))); if (score !== null) card.append(element("div", "insights-score", `${score}/100 · ${scoreLabel(score)}`)); card.append(element("p", "", t(interpretation))); const details = element("details", "insights-calculation"); details.append(element("summary", "", t("How this is calculated")), element("p", "", `${t("Scope")}: ${t("Current Board")} · ${t("Method")}: ${t(method)}`)); card.append(details); group.append(card); }); grid.append(group); }); diagnostics.append(grid); }
  const opportunities = section("insights-opportunities-title", "Opportunities", "Focus on the next useful improvements for this Canvas.");
  if (!unavailableReason) { if (!snapshot.findings.length) opportunities.append(element("p", "insights-diagnostic-state", t("No current findings"))); snapshot.findings.forEach((finding) => { const item = element("article", `insights-finding severity-${finding.severity}`); const severity = t(finding.severity === "critical" ? "High priority" : finding.severity === "important" ? "Worth improving" : "Minor improvement"); item.dataset.findingId = finding.code; item.append(element("span", "insights-severity", severity), element("h4", "", t(finding.title)), element("p", "", t(finding.description)), element("p", "insights-affected-count", `${finding.affectedNodeIds.length} ${t("Affected nodes")}`)); const list = element("ul", "insights-node-list"); finding.affectedNodeIds.slice(0, 3).forEach((id) => { const node = state.nodes.find((entry) => entry.id === id); if (node) list.append(element("li", "", `${String(node.title || node.type || t("Node")).slice(0, 160)} — ${String(node.type || t("Node")).slice(0, 80)}`)); }); if (finding.affectedNodeIds.length > 3) list.append(element("li", "", `+${finding.affectedNodeIds.length - 3} ${t("more")}`)); if (finding.affectedNodeIds.length) item.append(list); const actions = element("div", "insights-actions"); if (finding.affectedNodeIds.length) { const show = element("button", "fk-btn fk-btn-secondary", t("Show on Canvas")); show.type = "button"; show.setAttribute("aria-label", `${t("Show on Canvas")}: ${t(finding.title)}`); show.addEventListener("click", () => showInsightsFindingOnCanvas(finding.code)); actions.append(show); } if (state.user?.email && state.boardAccess?.canEdit === true && !state.publicBoardToken) { const ask = element("button", "fk-btn fk-btn-primary", t("Ask AI Brain")); ask.type = "button"; ask.addEventListener("click", () => askAiBrainAboutFinding(finding.code)); actions.append(ask); } item.append(actions); opportunities.append(item); }); }
  const methodology = section("insights-methodology-title", "Data and Methodology", "How to interpret these current-Canvas diagnostics."); const details = element("details", "insights-methodology"); details.append(element("summary", "", t("View data classifications, methods, and limitations"))); const classifications = element("dl", "insights-classifications"); [["Measured", "Verified observations from a supported data source."], ["Deterministic diagnostic", "Repeatable rules applied to the current Canvas."], ["Inferred", "A conclusion derived from disclosed Canvas rules."], ["Simulated", "An assumption-based scenario, not a measurement."], ["User-entered", "Information supplied directly by a user."], ["Unavailable", "No reliable value is currently available."]].forEach(([term, definition]) => { classifications.append(element("dt", "", t(term)), element("dd", "", t(definition))); }); const methodList = element("ul", "insights-method-list"); ["Canvas readiness formula", "Funnel-stage mapping", "Platform counting", "CTA variation check", "Distinct audience labels", "Distinct tone labels", "Landing Page trust-field presence"].forEach((method) => methodList.append(element("li", "", t(method)))); const meta = provenance("deterministic_diagnostic", "Structural checks"); details.append(classifications, element("h4", "", t("Current source")), element("p", "", `${meta.source} · ${meta.scope} · ${meta.canvasState} · ${meta.analyzedAt || ""} · ${state.currentBoardName || t("Current Board")} · ${state.nodes.length} ${t("Canvas nodes")} · ${sourceState}${snapshot?.analyzedAt ? ` · ${t("Last analyzed")}: ${new Date(snapshot.analyzedAt).toLocaleString()}` : ""}`), element("h4", "", t("Diagnostic methods")), methodList, element("h4", "", t("Assumptions")), element("p", "", t("Diagnostics use only the fields and supported node roles present in the current Canvas.")), element("h4", "", t("Limitations")), element("p", "", t("Canvas diagnostics evaluate the structure and content currently available in Funklix. They do not measure audience response, media delivery, conversions, revenue, or business impact."))); methodology.append(details);
}
