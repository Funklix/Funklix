...
// BW-32.2: deliberate, account-scoped LinkedIn connection controls. Settings reads a safe projection only.
(function initializeLinkedInSettings(){
  const card=document.getElementById('linkedin-connection-card'),connect=document.getElementById('linkedin-connect-button'),disconnect=document.getElementById('linkedin-disconnect-button'),status=document.getElementById('linkedin-connection-status'),detail=document.getElementById('linkedin-connection-detail'),result=document.getElementById('linkedin-connection-result');
  if(!card||!connect)return;let connectionId=null,busy=false;
  const outcomes={connected:'LinkedIn connected successfully.',limited:'LinkedIn connected with limited permissions.',cancelled:'Authorization was cancelled.',denied:'Connection could not be completed.',expired:'Authorization expired.',mismatch:'Reconnect required.',failed:'Connection could not be completed.'};
  const params=new URLSearchParams(location.search),returned=params.get('social');if(returned&&outcomes[returned]){result.textContent=outcomes[returned];history.replaceState(history.state,'',`${location.pathname}${location.hash}`);queueMicrotask(()=>result.focus());}
  ...
  connect.addEventListener('click',async()=>{if(busy)return;busy=true;connect.disabled=true;connect.textContent='Opening LinkedIn…';try{const response=await fetch('/api/social-connections-linkedin-start',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({returnPath:'/settings'})});const data=await response.json();if(!response.ok||!data.ok)throw new Error();location.assign(data.value.authorizationUrl);}catch{result.textContent='Connection could not be completed.';busy=false;connect.disabled=false;connect.textContent=connectionId?'Reconnect':'Connect LinkedIn';}});
  disconnect.addEventListener('click',async()=>{if(!connectionId||!confirm('Disconnect LinkedIn?'))return;disconnect.disabled=true;try{const response=await fetch('/api/social-connections-linkedin-disconnect',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({connectionId})});if(!response.ok)throw new Error();connectionId=null;disconnect.hidden=true;status.textContent='Not connected';detail.textContent='LinkedIn is disconnected.';connect.textContent='Connect LinkedIn';result.textContent='LinkedIn disconnected.';}catch{result.textContent='Connection could not be completed.';}finally{disconnect.disabled=false;}});
  document.getElementById('settings-dialog')?.addEventListener('toggle',refresh);refresh();
})();
