  ...
  if (el.aiBrainSummary) {
    el.aiBrainSummary.innerHTML = `
      <section class="ai-brain-wrap">
        <header class="ai-brain-header"><h3>🧠 AI Brain</h3><p>Your AI strategist & creative partner<br/><small>Last updated: ${state.analysisLastUpdatedAt ? new Date(state.analysisLastUpdatedAt).toLocaleTimeString() : "—"}${state.analysisError ? ` · ${state.analysisError}` : ""}</small></p><button type="button" id="refresh-ai-brain" ${state.analysisRefreshing ? "disabled" : ""}>${state.analysisRefreshing ? "Refreshing..." : "Refresh analysis"}</button></header>
    ...
    el.aiBrainSummary.querySelector("#refresh-ai-brain")?.addEventListener("click", async () => {
      ...
      try {
        ...
        state.analysisError = "Could not refresh analysis.";
      } finally {
