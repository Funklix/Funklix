...
(function initializeLinkedInSettings(){
  ...
  if(!card||!connect)return;let connectionId=null,busy=false,diagnostics='';
  ...
  const callbackKeys=['social','classification','phase','client_request_id','server_request_id','timestamp'];
  ...
  if(returned&&Object.hasOwn(outcomes,returned)){
    ...
    const values={timestamp:/^\d{4}-\d\d-\d\dT[\d:.]+Z$/.test(params.get('timestamp')||'')?params.get('timestamp'):'',outcome:returned,classification:safeWord(params.get('classification')),phase:safeWord(params.get('phase')),clientRequestId:safeId(params.get('client_request_id')),serverRequestId:safeId(params.get('server_request_id'))};
    result.textContent=language?.t?.(outcomes[returned])||outcomes[returned];diagnostics=Object.entries(values).filter(([,value])=>value).map(([key,value])=>`${key}: ${value}`).join('\n');if(copy){copy.hidden=!diagnostics;copy.addEventListener('click',()=>navigator.clipboard?.writeText(diagnostics));}
